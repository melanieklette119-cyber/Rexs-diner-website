...
export async function POST(request: Request) {
  try {
    ...
    if (!planId) return NextResponse.json({ error: "Bitte zuerst eine Mitgliedschaft auswählen." }, { status: 400 })
    const plan = await getMembershipPlan(planId)
    const fields = validateMembershipFields({ ...body, discordId })
    if (body.discordId && String(body.discordId).trim() !== discordId) return NextResponse.json({ error: "Die Discord-ID passt nicht zur Sitzung. Bitte die Seite neu laden." }, { status: 403 })
    const now = new Date()
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Vertrag konnte nicht erstellt werden." }, { status: 400 })
  }
}
